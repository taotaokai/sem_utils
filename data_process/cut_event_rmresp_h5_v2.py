...
logger.info(f"Processing event: {event_origin}")
...
# Initialize taup model and SDS client
...
logger.info("Querying waveforms...")
...
for network, station, location, channel in nslc_filtered:
    ...
    try:
        ...
        logger.warning(
            f"{err} - Failed to read {sxml_file}, station ignored. ({station_id})"
        )
        continue
    ...
    try:
        ...
        logger.warning(
            f"{err} - Cannot find station metadata for {seed_id}, station ignored. ({station_id})"
        )
        continue
    ...
    try:
        ...
        logger.warning(
            f"{err} - Failed to get waveforms, station ignored. ({station_id})"
        )
        continue
    ...
    for tr in st:
        if tr.stats.starttime > time_window_starttime:
            logger.warning(
                f"Trace starttime {tr.stats.starttime} is later than required {time_window_starttime}, {tr.id} ignored. ({station_id})"
            )
            traces_to_remove.append(tr)
        elif tr.stats.endtime < time_window_endtime:
            logger.warning(
                f"Trace endtime {tr.stats.endtime} is earlier than required {time_window_endtime}, {tr.id} ignored. ({station_id})"
            )
            traces_to_remove.append(tr)
    ...
    for tr in st:
        try:
            ...
            logger.warning(
                f"{err} - Cannot get instrument response, {tr.id} ignored. ({station_id})"
            )
            traces_to_remove.append(tr)
    for tr in traces_to_remove:
        st.remove(tr)
    ...
    for key in ["latitude", "longitude", "elevation", "local_depth"]:
        ...
        if not all([tr.stats.metadata[key] == val0 for tr in st[1:]]):
            logger.warning(
                f"Coordinates are not the same across all components, only one is kept. ({station_id})"
            )
            coords_match = False
            break
    ...
    for tr in st:
        try:
            ...
            logger.warning(
                f"{err} - Failed to evaluate instrument response, {tr.id} ignored. ({station_id})"
            )
            traces_to_remove.append(tr)
            continue
        # print(resp.get_paz())
        ...
        logger.debug(f"Number of zero zeros = {nz}")
    ...
    logger.info(
        f"Response corner frequency = [{resp_lc}, {resp_hc}] for {station_id}"
    )
    ...
    if len(st.select(component="Z")) != 1:
        logger.warning(f"No Z component, station ignored. ({station_id})")
        continue
    ...
    if len(horizontal_orientations) > 0:
        if horizontal_orientations != {"1", "2"} and horizontal_orientations != {
            ...
            if len(st1) != 1 or len(st2) != 1:
                ...
                if dip1 != 0 or dip2 != 0:
                    logger.warning(
                        f"Horizontal components' dip angles ({dip1}, {dip2}) != 0, ignored. ({station_id})"
                    )
                    traces_to_remove += [tr1, tr2]
                # if abs((az1 - az2)%180 / 90 - 1) > 1e-5:
                if abs((az1 - az2) % 360) < 10:
                    logger.warning(
                        f"Horizontal components' azimuth angles ({az1}, {az2}) are close to parallel, ignored. ({station_id})"
                    )
                    traces_to_remove += [tr1, tr2]
    for tr in traces_to_remove:
        if tr in st:
    ...
    if config["enforce_3comp"] and len(st) != 3:
        logger.warning(f"Not exactly 3 components, station ignored. ({station_id})")
        continue

    station_name = f"{network}_{station}"
    if station_name in groot:
        logger.warning(f"{station_name} already exists, overwrite. ({station_id})")
    ...
    ca.attrs["starttime"] = resample_starttime
