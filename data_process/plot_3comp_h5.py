...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(
        description="plot 3-component seismograms from h5 file"
    )

    parser.add_argument("h5_file", help="data h5 file")
    parser.add_argument("out_pdf", help="output PDF file")
    parser.add_argument(
        "--cmt_file", default=None, help="CMTSOLUTOIN file for traveltime calculation"
    )
    parser.add_argument(
        "--data_tag", default="DATA_DISP", help="array name to plot for each station"
    )
    parser.add_argument(
        "--bin_size", type=float, default=10.0, help="azimuthal bin size (degree)"
    )
    parser.add_argument(
        "--max_nsta",
        type=int,
        default=20,
        help="maximum number of stations in each azimuthal bin",
    )
    parser.add_argument(
        "--freq_limit",
        nargs=2,
        type=float,
        default=None,
        help="frequency band for filtering (Hz)",
    )
    parser.add_argument(
        "--reduce_rayp",
        type=float,
        default=12.0,
        help="ray parameter for reducing time window (s/deg)",
    )
    parser.add_argument(
        "--time_limit",
        nargs=2,
        type=float,
        default=[-100, 1000],
        help="time window for plotting (sec) in reduced time",
    )
    parser.add_argument(
        "--min_SNR",
        type=float,
        default=None,
        help="SNR threshold to grayed out plot of seismogram",
    )
    parser.add_argument(
        "--noise_twin",
        nargs=2,
...
def plot_seismogram_3comp(
    ...
    reduce_rayp=10.0,  # reduced time = time_after_origin - dist_degree * rayp (sec/degree)
    ...
    parallels = np.arange(-90.0, 90, 20.0)
    meridians = np.arange(0.0, 360, 20.0)
    lat0center, lon0center, mapWidth, mapHeight = centerMap(
    ...
    while ista < len(stations):
        ...
        for i in range(3):
            ax = ax_RTZ[i]
            ax.plot(
                [x[1] - reduce_rayp * x[0] for x in ttcurve_p],
            ...
            ax.plot(
                [x[1] - reduce_rayp * x[0] for x in ttcurve_s],
        ...
        for sta in stations_azimuthal_bin:
            ...
            if len(ind_zcomp) != 1:
                print(
                    f"[WARN] not exactly one vertical component in {data_orientation}, skip {sta}"
                )
                continue
            if len(ind_hcomp) != 0 and len(ind_hcomp) != 2:
                print(
                    f"[WARN] not exactly zero or two horizontal components in {data_orientation}, skip {sta}"
                )
                continue
            ...
            if len(ind_hcomp) == 2:
                ...
                proj_EN2RT = np.array([[sin_az, cos_az], [cos_az, -sin_az]])
                # print(proj_EN_to_H12, proj_H12_to_EN, proj_EN2RT)
                data_RT = np.dot(
                    proj_EN2RT, np.dot(proj_H12_to_EN, h5data[ind_hcomp, :])
                )

            # apply bandpass filter
            if freq_limit:
                sos = signal.butter(
                    4, freq_limit, "bp", fs=data_sampling_rate, output="sos"
                )
            ...
            data_t0 = data_starttime - event_origin.time
            times = (
                np.arange(data_npts) / data_sampling_rate + data_t0
            )  # times after origin
