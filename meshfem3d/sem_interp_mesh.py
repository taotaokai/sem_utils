...
from meshfem3d_utils import get_gll_weights, sem_mesh_read, sem_mesh_locate_points
...
for iproc_target in range(mpi_rank, nproc_target, mpi_size):
    ...
    for iproc_source in range(nproc_source):
        ...
        ii = (
            (status_gll_target != 1)
            & (status_all != -1)
            & ((misloc_all < misloc_gll_target) | (status_all == 1))
        )
        ...
        for ipoint in ipoint_select:
            ...
            model_gll_target[:, ipoint] = np.sum(
    ...
    for imodel in range(nmodel):
        ...
        with FortranFile(model_file, "w") as f:
            f.write_record(
                np.array(
                    np.ravel(model_gll_target[imodel, :, :, :, :], order="F"),
                    dtype="f4",
                )
            )
