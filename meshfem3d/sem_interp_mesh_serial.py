...
for iproc_target in [4, ]:
  ...
  for iproc_source in [4, ]:
    ...
    print("sem_locate_points_hex27 elapse", time.time() - tic1)
    ...
    #model_gll_target[:,ii] = np.sum(source_model_gll[:,:,:,:,ispec_all[ii]]
    #    * hlagx[:,None,None,:]
    #    * hlagy[None,:,None,:]
    #    * hlagz[None,None,:,:], axis=(1,2,3))
    #source_model_gll = np.sum(source_model_gll[:,:,:,:,ispec_all[ii]] * hlagx[None,:,None,None,:], axis=1) 
    #source_model_gll = np.sum(source_model_gll * hlagy[None,:,None,:], axis=1) 
    #model_gll_target[:,ii] = np.sum(source_model_gll * hlagz[None,:,:], axis=1) 
    #model_gll_target[:,ii] = np.sum( 
    #    np.sum( 
    #      np.sum(source_model_gll[:,:,:,:,ispec_all[ii]] * hlagx[:,None,None,:], axis=1) 
    #      * hlagy[:,None,:], axis=1)
    #    * hlagz, axis=1)
