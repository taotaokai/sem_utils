...
def parse_arguments():
    ...
    parser.add_argument(
        "--sigma_km", type=float, default=10.0, help="one sigma of Gaussian mask in km"
    )
    parser.add_argument(
        "--reverse", action="store_true", help="reverse mask to 1.0 - mask"
    )
    parser.add_argument("out_dir", help="output dir for proc*_reg1_mask.bin")
...
def main():
    ...
    for iproc in range(rank_world, args.nproc, size_world):
        ...
        with FortranFile(out_file, "w") as f:
            f.write_record(np.array(weight_mask[ibool], dtype="f4"))
