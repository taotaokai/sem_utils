...
for iproc in range(args.procnum_begin, args.procnum_end + 1):
    ...
    for ispec in range(nspec):
        ...
        iglob_list = [  # 8 corners of each hex cell
            ibool[0      ,       0,       0, ispec] - 1,  # -1 so index begins from 0
            ibool[NGLLX-1,       0,       0, ispec] - 1,
            ibool[NGLLX-1, NGLLY-1,       0, ispec] - 1,
            ibool[      0, NGLLY-1,       0, ispec] - 1,
            ibool[0      ,       0, NGLLZ-1, ispec] - 1,
            ibool[NGLLX-1,       0, NGLLZ-1, ispec] - 1,
            ibool[NGLLX-1, NGLLY-1, NGLLZ-1, ispec] - 1,
            ibool[      0, NGLLY-1, NGLLZ-1, ispec] - 1,
        ]
    ...
    cell_data_total = np.hstack((cell_data_total, idoubling))

# write out vtk
grid = pv.UnstructuredGrid({pv.CellType.HEXAHEDRON: cell_topo_total}, point_xyz_total)
grid.cell_data['idoubling'] = cell_data_total
