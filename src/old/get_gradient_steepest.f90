...
! calculates gradient by steepest descent method
  ...
  real(kind=CUSTOM_REAL):: weight ! taok: add
    ...
    do k = 1, NGLLZ
      do j = 1, NGLLY
        do i = 1, NGLLX
          ...
          ! taok: get radius of point
          iglob = ibool(i,j,k,ispec)
          r = sqrt( x(iglob)*x(iglob) + y(iglob)*y(iglob) + z(iglob)*z(iglob) )

          ! taok: depth weighting (body wave: honor deeper part of kernel)
          weight = 1.0
          if (USE_DEPTH_WEIGHTED_KERNEL) then
            if (r >= R_STOP) weight = 0.0
            if (r < R_STOP .and. r > R_PASS ) then
              weight = 0.5 * (1 + cos(PI*(r-R_PASS)/(R_STOP-R_PASS)))
            endif
          endif
          ...
          model_dbulk(i,j,k,ispec) = - kernel_bulk(i,j,k,ispec) * weight
          ! for shear
          !model_dbeta(i,j,k,ispec) = - kernel_beta(i,j,k,ispec)
          model_dbeta(i,j,k,ispec) = - kernel_beta(i,j,k,ispec) * weight
          ! for rho
          !model_drho(i,j,k,ispec) = - kernel_rho(i,j,k,ispec)
          model_drho(i,j,k,ispec) = - kernel_rho(i,j,k,ispec) * weight
          ...
          if (USE_DEPTH_RANGE_MAXIMUM) then
            ...
            if (r < R_TOP .and. r > R_BOTTOM) then
              ...
              max_beta = abs( model_dbeta(i,j,k,ispec) ) ! taok: modified
