...
! ================ Specification Part ===============
  ...
  use constants,only: CUSTOM_REAL,MAX_STRING_LEN,IIN,IOUT, &
    ...
    NSPEC_CRUST_MANTLE,NGLOB_CRUST_MANTLE,REG,NPROCTOT_VAL
...
private
  ...
  ! operation name
  character(len=MAX_STRING_LEN) :: opname

  ! kernel input
  integer :: kernel_num
  character(len=MAX_STRING_LEN), dimension(:), allocatable :: &
    kernel_dirs(:), kernel_names(:)
  real(kind=CUSTOM_REAL), dimension(:,:,:,:,:), allocatable :: kernels

  ! mesh topo
  ! note: assume all kernel files share the same mesh topo
  character(len=MAX_STRING_LEN) :: topo_dir

  ! kernel output
  logical :: L_COMPUTE_KERNEL_INTEGRAL = .false.
  logical :: L_WRITE_KERNEL_OUT = .false.
  real(kind=CUSTOM_REAL) :: kernel_out_integral, volume_integral
  real(kind=CUSTOM_REAL), dimension(:,:,:,:), allocatable :: kernel_out
  character(len=MAX_STRING_LEN) :: kernel_out_dir, kernel_out_name
  ...
  ! flag if wgll_cube has been computed
  logical :: L_GLL_WEIGHT_COMPUTED = .false.

  !---- public arrays
  public :: MAX_STRING_LEN, IIN
  public :: opname, kernel_num, kernel_dirs, kernel_names, topo_dir
  public :: kernel_out_dir, kernel_out_name, kernel_out_integral, volume_integral
  public :: L_WRITE_KERNEL_OUT, L_COMPUTE_KERNEL_INTEGRAL

  !---- public routines
  public :: sem_math_op, sem_check
