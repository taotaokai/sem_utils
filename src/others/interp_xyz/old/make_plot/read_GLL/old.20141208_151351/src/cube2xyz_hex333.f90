! map a given point (xi,eta,gamma) in the reference cube to physical space (xyz),
! and also output Jacobian (Dxi/Dx) for 3x3x3 elements

subroutine cube2xyz_hex333 &
( xa,ya,za, xi,eta,gamma, &
  x,y,z, jacobian,xix,xiy,xiz,etax,etay,etaz,gammax,gammay,gammaz )
	...
	! number of dimensions and control nodes
	!integer, parameter :: NDIM=3, NGNOD=27
	
	double precision :: x,y,z,xix,xiy,xiz,etax,etay,etaz,gammax,gammay,gammaz
	double precision :: xi,eta,gamma,jacobian

	! coordinates of the anchor points of 3x3x3 hex
	...
	do ia = 1,NGNOD
	  ! sum[N_ijk(xi,eta,gamma)*X_ijk,{i,j,k}]
	  x=x+shape3D(ia)*xa(ia)
	  y=y+shape3D(ia)*ya(ia)
	  z=z+shape3D(ia)*za(ia)
	
	  ! sum[D[N_ijk,xi]*X_ijk,{i,j,k}]
	  xxi=xxi+dershape3D(1,ia)*xa(ia)
	  yxi=yxi+dershape3D(1,ia)*ya(ia)
	  zxi=zxi+dershape3D(1,ia)*za(ia)
	
	  ! sum[D[N_ijk,eta]*X_ijk,{i,j,k}]
	  xeta=xeta+dershape3D(2,ia)*xa(ia)
	  yeta=yeta+dershape3D(2,ia)*ya(ia)
	  zeta=zeta+dershape3D(2,ia)*za(ia)
	
	  ! sum[D[N_ijk,gamma]*X_ijk,{i,j,k}]
	  xgamma=xgamma+dershape3D(3,ia)*xa(ia)
	  ygamma=ygamma+dershape3D(3,ia)*ya(ia)
	  zgamma=zgamma+dershape3D(3,ia)*za(ia)
	enddo
...
end subroutine cube2xyz_hex333
