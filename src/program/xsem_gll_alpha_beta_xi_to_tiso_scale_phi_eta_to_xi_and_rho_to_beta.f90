subroutine selfdoc()
  ...
  print '(a)', "    <nproc> <mesh_dir> <model_dir> <scale_phi> <scale_eta> <scale_rho> <out_dir>"
  print '(a)', ""
  print '(a)', "DESCRIPTION"
  print '(a)', ""
  print '(a)', ""
  ...
  print '(a)', "  (string) model_dir:  directory holds proc*_reg1_[alpha,beta,xi,vp0,vs0,rho0].bin"
  print '(a)', "  (string) out_dir:  output directory for proc*_reg1_[vph,vpv,vsv,vsh,eta,rho].bin"
  print '(a)', ""
  print '(a)', "NOTE"
  print '(a)', ""
...
program xsem_gll_alpha_beta_xi
  ...
  integer :: ispec, nspec
  ! input model
  real(dp), dimension(:,:,:,:), allocatable :: vp0,vs0,rho0
  real(dp), dimension(:,:,:,:), allocatable :: alpha,beta,xi
  ! output model
  real(dp), dimension(:,:,:,:), allocatable :: phi,eta,vp,vs
  real(dp), dimension(:,:,:,:), allocatable :: vph,vpv,vsh,vsv,rho
  ...
  read(args(4),*) scale_phi
  read(args(5),*) scale_eta 
  read(args(6),*) scale_rho 
  ...
  do iproc = myrank, (nproc-1), nrank
    ...
    ! sacle phi,eta,rho to xi
    phi = scale_phi * xi
    eta = 1.0 - scale_eta * xi
    rho = (1.0 + scale_rho*beta) * rho0
