subroutine selfdoc()
  ...
  print '(a)', "NAME"
  print '(a)', ""
  print '(a)', "  xsem_kernel_aijkl_to_dvpv_dvsv_thomsen"
  print '(a)', "    - reduce aijkl kernel to radial anisotropy kernel (dvpv,dvsv,eps,gamma,delta)"
  print '(a)', ""
  print '(a)', "SYNOPSIS"
  print '(a)', ""
  print '(a)', "  xsem_kernel_aijkl_to_dvpv_dvsv_thomsen \"
  ...
  print '(a)', "  (string) model_dir:  directory holds the model files, proc*_reg1_[vpv_ref,vsv_ref,dvpv,dvsv,eps,gamma,delta].bin"
  print '(a)', "  (string) kernel_dir:  directory holds the aijkl_kernel files"
  print '(a)', "                        proc******_reg1_aijkl_kernel.bin"
  print '(a)', "  (string) out_dir:  output directory for [dvpv,dvsv,eps,gamma,delta]_kernel"
  print '(a)', ""
  print '(a)', "NOTE"
  print '(a)', ""
  ...
  print '(a)', "     delta = (C13 + 2*C44)/C33 - 1 (first order approximation of C13 in delta)"
  print '(a)', "  4. vpv = (1 + dvpv)*vpv_ref, vsv = (1 + dvsv)*vsv_ref"

end subroutine


!///////////////////////////////////////////////////////////////////////////////
program xsem_kernel_aijkl_to_dvpv_dvsv_thomsen
  ...
  ! model gll
  real(dp), dimension(:,:,:,:), allocatable :: vpv_ref, vsv_ref, vpv2, vsv2
  real(dp), dimension(:,:,:,:), allocatable :: dvpv, dvsv, eps, gamma, delta

  ! kernel gll 
  real(dp), allocatable :: aijkl_kernel(:,:,:,:,:)
  real(dp), dimension(:,:,:,:), allocatable :: dvpv_kernel, dvsv_kernel
  ...
  allocate(dvpv_kernel(NGLLX,NGLLY,NGLLZ,nspec))
  allocate(dvsv_kernel(NGLLX,NGLLY,NGLLZ,nspec))
  ...
  do iproc = myrank, (nproc-1), nrank
    ...
    ! reduce kernels
    dvpv_kernel = ( aijkl_kernel(1,:,:,:,:)*(1.0+2.0*eps) &
                  + aijkl_kernel(7,:,:,:,:)*(1.0+2.0*eps) &
                  + aijkl_kernel(12,:,:,:,:)              &
                  + aijkl_kernel(2,:,:,:,:)*(1.0+2.0*eps) &
                  + aijkl_kernel(3,:,:,:,:)*(1.0+delta)   &
                  + aijkl_kernel(8,:,:,:,:)*(1.0+delta)   &
                  ) * 2.0 * vpv_ref**2 * (1+dvpv)

    dvsv_kernel = ( aijkl_kernel(2,:,:,:,:)*(-2.0*(1.0+2.0*gamma)) &
                  + aijkl_kernel(3,:,:,:,:)*(-2.0)                 &
                  + aijkl_kernel(8,:,:,:,:)*(-2.0)                 &
                  + aijkl_kernel(16,:,:,:,:)                       &
                  + aijkl_kernel(19,:,:,:,:)                       &
                  + aijkl_kernel(21,:,:,:,:)*(1.0+2.0*gamma)       &
                  ) * 2.0 * vsv_ref**2 * (1+dvsv)

    eps_kernel = ( aijkl_kernel(1,:,:,:,:) &
                 + aijkl_kernel(2,:,:,:,:) &
                 + aijkl_kernel(7,:,:,:,:) &
                 ) * 2.0 * vpv_ref**2 * (1+dvpv)**2

    gamma_kernel = ( aijkl_kernel(21,:,:,:,:)        &
                   + aijkl_kernel(2,:,:,:,:)*(-2.0)  &
                   ) * 2.0 * vsv_ref**2 * (1+dvsv)**2

    delta_kernel = ( aijkl_kernel(3,:,:,:,:) &
                   + aijkl_kernel(8,:,:,:,:) &
                   ) * vpv_ref**2 * (1+dvpv)**2
    ...
    print *, "dvpv_kernel: min/max=", minval(dvpv_kernel), maxval(dvpv_kernel)
    print *, "dvsv_kernel: min/max=", minval(dvsv_kernel), maxval(dvsv_kernel)
    ...
    call sem_io_write_gll_file_1(out_dir, iproc, iregion, 'dvpv_kernel', dvpv_kernel)
    call sem_io_write_gll_file_1(out_dir, iproc, iregion, 'dvsv_kernel', dvsv_kernel)
