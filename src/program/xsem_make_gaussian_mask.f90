subroutine selfdoc()
  print '(a)', "NAME"
  print '(a)', ""
  print '(a)', "  xsem_make_gaussian_mask "
  ...
  print '(a)', "  (string) source_xyz_list: list of source locations (x,y,z)(normalized by EARTH_R) and gauss_width(km) "
...
program xsem_make_kernel_mask
  ...
  real(dp), allocatable :: source_xyz(:,:), gauss_width(:)
  ! mesh
  ...
  if (myrank == 0) then
    ...
    print *, "#[LOG] xyz_list=", trim(source_xyz_list)
  ...
  allocate(source_xyz(3, nsource), gauss_width(nsource))
  ! read source x,y,z 
  do isrc = 1, nsource
    read(lines(isrc), *) source_xyz(1, isrc), source_xyz(2, isrc), source_xyz(3, isrc), gauss_width(isrc)
  enddo

  !===== loop each mesh slice

  ! non-dimensionalize
  gauss_width = abs(gauss_width)/EARTH_R_KM
