subroutine selfdoc()
  ...
  print '(a)', "NAME"
  print '(a)', ""
  print '(a)', "  xsem_sum_event_kernels_aijkl"
  ...
  print '(a)', "  (string) kernel_dir_list:  list of event kernel directories (proc*_reg1_aijkl_kernel.bin)"
  ...
  print '(a)', "  (string) out_dir:  output directory of summed aijkl kernel"
  print '(a)', ""
  print '(a)', "NOTE"
  print '(a)', ""
...
!//////////////////////////////////////////////////////////////////////////////
program xsem_sum_event_kernels_aijkl
  ...
  real(dp), allocatable :: aijkl_kernel_sum(:,:,:,:,:)
  ...
  allocate(aijkl_kernel_sum(21,NGLLX,NGLLY,NGLLZ,nspec))
  ...
  do iproc = myrank, (nproc-1), nrank
    ...
    aijkl_kernel_sum = 0.0_dp
    do iker = 1, nkernel

      ! read kernel gll
      call sem_io_read_cijkl_kernel(kernel_dirs(iker), iproc, iregion, 'aijkl_kernel', aijkl_kernel)
      ...
      ! sum up kernels 
      aijkl_kernel_sum = aijkl_kernel_sum + aijkl_kernel*spread(mask, 1, 21)

    enddo ! iker

    ! write out cijkl kernel
    call sem_io_write_cijkl_kernel(out_dir, iproc, iregion, 'aijkl_kernel', aijkl_kernel_sum)
