...
def _get_resp_corner_freq(freqs, resp_amp, ref_freq=-1, threshold_dB=3):
...
groot._v_attrs['location'] = {'longitude': event_origin.longitude,
...
for net, sta in stations:
    ...
    for nslc in nslc_list:
        ...
        inv1 = inv.select(network=net, station=sta, location=loc, channel=cha, time=event_origin.time)
        if not inv1:
    ...
    if nslc_use:
        ...
        nslc_filtered.append(nslc_use[-1][0]) # only keep the trace with largest coverage for each station
...
for network, station, location, channel in nslc_filtered:
    ...
    inv = inv.select(network=network, station=station,
                     location=location, channel=channel[:-1]+'?',
                     time=event_origin.time)
    if not inv:
        print(f"[WARN] no station metadata on {event_origin.time}), station ignored. ({station_id})\n")
        continue
    ...
    try:
        ...
        st = client.get_waveforms(network, station, location, channel[0:2]+'?',
                                  time_window_starttime - pad_time, time_window_endtime + pad_time, merge=-1)
    except Exception as err:
    ...
    print(f'[INFO] response corner frequency = [{resp_lc}, {resp_hc}] for {station_id}\n')
    ...
    resample_npts = int((time_window_endtime - time_window_starttime) * config_sampling_rate)
    for tr in st:
        fs = tr.stats.sampling_rate
        npts = tr.stats.npts
        ...
        tr.interpolate(config_sampling_rate,
    ...
    channels = [(tr.stats.channel, tr.stats.metadata['azimuth'], tr.stats.metadata['dip']) for tr in st]
