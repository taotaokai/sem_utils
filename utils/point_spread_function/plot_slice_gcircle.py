...
model_names = ['dlnv', 'alpha_Hdm_precond', 'beta_Hdm_precond']
...
# filp the xsection left and right sides
flip = 1
...
for irow in range(nrow):
  ...
  # colorbar axis
  cax = fig.add_axes([0.13+width, 0.1+irow*subplot_height, 0.01, 0.8*subplot_height])
  ...
  if tag == 'dlnv':
    ...
    cb = plt.colorbar(cs, cax=cax, ticks=np.arange(-1,1.1,0.5), orientation="vertical")
    #cb.set_label('%', fontsize=10)
    cb.ax.set_title('(%)', fontsize=10)

  #if tag == 'dlnvs_kernel_precond':
  if tag in ['alpha_Hdm_precond', 'beta_Hdm_precond']:
    cmap = plt.cm.get_cmap("jet_r")
    #cmap = plt.cm.get_cmap("seismic_r")
    zz = model[tag]
    norm_amp = np.max(zz)
    ...
    cs = ax.contour(xx, yy, zz_dlnv, levels=levels, colors=('w',), linewidths=(0.1,))
  ...
  # surface marker
  marker_theta = np.linspace(theta[0], theta[-1], nmarker)
  x = flip * np.sin(marker_theta) * radius[-1]
  y = np.cos(marker_theta) * radius[-1]
  ax.plot(x, y, 'ko', markersize=4, clip_on=False)
  x1 = flip * np.sin(theta[-1]) * radius[-1]
  x2 = flip * np.sin(theta[-2]) * radius[-1]
