      ...
      do k = 1, NGLLZ
        do j = 1, NGLLY
          do i = 1, NGLLX
            ...
            ! taok: approximate diagonal Hessian: illumination amplitude
            hess_kl_crust_mantle(i,j,k,ispec) = max(hess_kl_crust_mantle(i,j,k,ispec), &
              sqrt(sum(b_disp_crust_mantle(:,iglob)**2) * sum(disp_crust_mantle(:,iglob)**2)))
