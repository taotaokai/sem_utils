...
! by Ebru
  ...
  double precision :: MSEC ! taok: added
...
! double precision :: stcolat, evcolat ! taok: added
  real :: DEPMEN
...
!----------------------------------------------------------------------
  ...
  ! taok: add gcarc
! call lat_2_geocentric_colat_dble(stlat(irec),stcolat)
! call lat_2_geocentric_colat_dble(cmt_lat,evcolat)
! GCARC = sngl(acos(cos(evcolat)*cos(stcolat)+sin(evcolat)*sin(stcolat)*cos((EVLO-STLO)*DEGREES_TO_RADIANS)) * RADIANS_TO_DEGREES )
  ...
  ! taok: modfied to account for sub milliseconds
  NZSEC  = int(sec+t_shift)
! NZMSEC = int((sec+t_shift-int(sec+t_shift))*1000)
  MSEC = (sec+t_shift-int(sec+t_shift))*1000
  NZMSEC = int(MSEC)
