...
$sem_utils/bin/xmap_gcircle \
    $mesh_lat0 $mesh_lon0 \
    $az theta.list > trench_parallel_center.list
...
$sem_utils/bin/xmap_gcircle \
    $mesh_lat0 $mesh_lon0 \
    $az theta.list > trench_normal_center.list
...
# trench parallel gcircles
echo "# trench parallel gcircles:" >> $out_file

az=26.5; theta0=-20; theta1=20; ntheta=401
cat trench_parallel_center.list |\
while read theta lat lon
do
    printf "%6.2f %7.2f %05.1f %6.2f %6.2f %4d %7.2f %7.2f %4d trench_parallel_lon%05.1f\n" \
        $lat $lon $az $theta0 $theta1 $ntheta $r0 $r1 $nr $lon
done >> $out_file
...
while read theta lat lon
do
    printf "%6.2f %7.2f %05.1f %6.2f %6.2f %4d %7.2f %7.2f %4d trench_normal_lat%04.1f\n" \
        $lat $lon $az $theta0 $theta1 $ntheta $r0 $r1 $nr $lat
done >> $out_file
