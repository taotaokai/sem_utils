...
for irow in range(nrow):
  ...
  if tag == 'eta':
    ...
    zz = model[tag]
    cs = ax.contourf(xx, yy, zz, cmap=cmap, levels=np.arange(0.9,1.1,0.02), extend="both")
  ...
  if tag in ['phi','xi']:
    cmap = plt.cm.get_cmap("jet")
    zz = model[tag]*100
    cs = ax.contourf(xx, yy, zz, cmap=cmap, levels=np.arange(-10,10.001,0.5), extend="both")
    cs.cmap.set_over('purple')
    cs.cmap.set_under('black')
    cb = plt.colorbar(cs, cax=cax, orientation="vertical")
    cb.ax.set_title("(%)", fontsize=10)
