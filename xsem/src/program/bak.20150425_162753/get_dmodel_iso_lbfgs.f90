...
'NAME', &
'', &
'  get_dmodel_iso_lbfgs - get isotropic model update', &
'', &
'SYNOPSIS', &
'', &
'  get_dmodel_iso_lbfgs <mesh_dir> <dmodel_dkernel_list> <kernel_dir> <out_dir> <log_file>', &
'', &
'DESCRIPTION', &
'', &
...
'  <log_file>  output log file (append if exists)', &
''
...
program get_dmodel_iso_lbfgs
  ...
  if (myrank == 0) then
    ...
    write(IOUT,*)
    write(IOUT,*) '#==========================================#' 
    write(IOUT,*) '# get_dmodel_iso_lbfgs:', trim(timestamp()) 
    write(IOUT,*) '#==========================================#' 
    write(IOUT,*)
  end if
