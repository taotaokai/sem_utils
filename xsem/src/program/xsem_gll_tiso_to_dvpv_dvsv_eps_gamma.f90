subroutine selfdoc()
  ...
  print '(a)', "    - convert GLL(vph,vpv,vsv,vsh) model into (dvpv,dvsv,eps,gamma,delta)"
  ...
  print '(a)', "  (string) model_dir:  directory holds proc*_reg1_[vsv_ref,vpv_ref,vph,vpv,vsv,vsh].bin"
  print '(a)', "  (string) out_dir:  output directory for proc*_reg1_[dvpv,dvsv,eps,gamma].bin"
...
program xsem_gll_tiso_to_dvpv_dvsv_eps_gamma
  ...
  integer :: ispec, nspec
  ! model
  real(dp), dimension(:,:,:,:), allocatable :: vsv_ref,vpv_ref,vph,vpv,vsh,vsv
  real(dp), dimension(:,:,:,:), allocatable :: dvpv,dvsv,eps,gamma
