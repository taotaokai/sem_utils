subroutine selfdoc()
  ...
  print '(a)', "NAME"
  print '(a)', ""
  print '(a)', "  xsem_slice_small_gcircle - create slice of SEM model along small circle based on rotation pole"
  print '(a)', ""
  print '(a)', "SYNOPSIS"
  print '(a)', ""
  print '(a)', "  xsem_slice_gcircle \"
  print '(a)', "    <nproc> <mesh_dir> <model_dir> <model_tags> "
  print '(a)', "    <lat0> <lon0> <lat_rot> <lon_rot> "
  ...
  print '(a)', "DESCRIPTION"
  print '(a)', ""
  print '(a)', "  create a verical slice of SEM model along specified small circle" 
...
program xsem_vertical_slice
  ...
  real(dp) :: lat0, lon0, lat_rot, lon_rot, theta0, theta1, radius0, radius1
  ...
  character(len=*), parameter :: lat_rot_name = "latitude_rotation"
  ...
  read(args(10), *) theta1 
  read(args(11), *) ntheta
  read(args(12), *) radius0
  read(args(13), *) radius1
  read(args(14), *) nradius
  read(args(15), *) flag_ellipticity
  read(args(16), '(a)') out_file
  ...
  lat_rot = lat_rot * DEGREES_TO_RADIANS
  lon_rot = lon_rot * DEGREES_TO_RADIANS
  ...
  ! unit radial vector v0 from earth's center to the origin point,
  ! and vrot for the rotation pole
  if (flag_ellipticity == 0) then
    ...
    vrot(1) = cos(lat_rot)*cos(lon_rot)
    vrot(2) = cos(lat_rot)*sin(lon_rot)
    vrot(3) = sin(lat_rot)
  else 
    ! WGS84 ellipsoid
    ...
    call geographic_lla2ecef(lat_rot, lon_rot, 0.0_dp, vrot(1), vrot(2), vrot(3))
    vrot = vrot / sqrt(sum(vrot**2))
  endif
  ...
  if (myrank == 0) then
    ...
    call check( nf90_def_var(ncid, lat_rot_name, NF90_DOUBLE, lat_rot_varid) )
    call check( nf90_put_att(ncid, lat_rot_varid, UNITS,      lat_rot_units) )
